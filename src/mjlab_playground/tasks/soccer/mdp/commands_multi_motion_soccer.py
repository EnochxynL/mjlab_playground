...
# MJLab: isaaclab.sim as sim_utils → not needed (MuJoCo handles physics natively)
from mjlab.entity import Entity  # MJLab: isaaclab.assets Articulation, RigidObject → mjlab.entity Entity
from mjlab.managers import CommandTerm, CommandTermCfg  # MJLab: isaaclab.managers → mjlab.managers
# MJLab: isaaclab.markers VisualizationMarkers, VisualizationMarkersCfg → not available
# MJLab: isaaclab.markers.config FRAME_MARKER_CFG → not available
# MJLab: isaaclab.utils configclass → using dataclasses.dataclass instead
from mjlab.utils.lab_api.math import (  # MJLab: isaaclab.utils.math → mjlab.utils.lab_api.math
    quat_apply,
...
class MotionCommand(CommandTerm):
    ...
    def __init__(self, cfg: MotionCommandCfg, env: ManagerBasedRlEnv):
        ...
        self.robot: Entity = env.scene[cfg.entity_name]  # MJLab: cfg.asset_name → cfg.entity_name; Articulation → Entity
        self.soccer_ball: Entity | None = None  # MJLab: RigidObject → Entity
        ...
        # MJLab: VisualizationMarkers → not available; target_point_marker / target_destination_marker omitted
        marker_cfg = cfg.target_point_marker_cfg
        self.target_point_marker = None  # MJLab: VisualizationMarkers(marker_cfg) → not available
        dest_marker_cfg = getattr(cfg, "target_destination_marker_cfg", None)
        self.target_destination_marker = None  # MJLab: VisualizationMarkers(dest_marker_cfg) → not available
    ...
    def robot_body_pos_w(self) -> torch.Tensor:
        return self.robot.data.body_link_pos_w[:, self.body_indexes]  # MJLab: body_pos_w → body_link_pos_w

    @property
    def robot_body_quat_w(self) -> torch.Tensor:
        return self.robot.data.body_link_quat_w[:, self.body_indexes]  # MJLab: body_quat_w → body_link_quat_w
    ...
    def robot_body_ang_vel_w(self) -> torch.Tensor:
        return self.robot.data.body_link_ang_vel_w[:, self.body_indexes]  # MJLab: body_ang_vel_w → body_link_ang_vel_w

    @property
    def robot_anchor_pos_w(self) -> torch.Tensor:
        return self.robot.data.body_link_pos_w[:, self.robot_anchor_body_index]  # MJLab: body_pos_w → body_link_pos_w

    @property
    def robot_anchor_quat_w(self) -> torch.Tensor:
        return self.robot.data.body_link_quat_w[:, self.robot_anchor_body_index]  # MJLab: body_quat_w → body_link_quat_w
    ...
    def robot_anchor_ang_vel_w(self) -> torch.Tensor:
        return self.robot.data.body_link_ang_vel_w[:, self.robot_anchor_body_index]  # MJLab: body_ang_vel_w → body_link_ang_vel_w
    ...
    def _update_target_points(self, env_ids: Sequence[int] | torch.Tensor):
        ...
        # MJLab: VisualizationMarkers → not available; target_point_marker visualization omitted
        if self.target_point_marker is not None:
            ...
            # self.target_point_marker.visualize(world_positions)  # MJLab: VisualizationMarkers.visualize → not available
    ...
    def _update_target_points_from_sim(self):
        ...
        ball_world_pos = self.soccer_ball.data.root_link_pos_w  # MJLab: root_pos_w → root_link_pos_w
        ...
        # MJLab: VisualizationMarkers → not available; marker visualization omitted
        if self.target_point_marker is not None:
            pass
            # self.target_point_marker.visualize(ball_world_pos)  # MJLab: VisualizationMarkers.visualize → not available
    ...
    def _update_destination_points(self, env_ids: Sequence[int] | torch.Tensor):
        ...
        # MJLab: VisualizationMarkers → not available; destination_marker visualization omitted
        if self.target_destination_marker is not None:
            ...
            # self.target_destination_marker.visualize(world_destination)  # MJLab: VisualizationMarkers.visualize → not available
    ...
    # MJLab: _set_debug_vis_impl and _debug_vis_callback removed — VisualizationMarkers not available in MJLab
    # IsaacLab uses Omniverse VisualizationMarkers for debug rendering; MJLab uses MuJoCo viewer.


# MJLab: @configclass → dataclass
from dataclasses import dataclass, field

@dataclass(kw_only=True)  # MJLab: kw_only=True for compatibility with CommandTermCfg keyword-only args
class MotionCommandCfg(CommandTermCfg):
    """Configuration for the motion command."""

    # MJLab: class_type not needed (cfg.build is used instead)
    # class_type: type = MotionCommand

    # MJLab: IsaacLab cfg fields below
    entity_name: str = MISSING  # MJLab: asset_name → entity_name
    ...
    resampling_time_range: tuple[float, float] = (1.0e9, 1.0e9)  # MJLab: required by CommandTermCfg
    debug_vis: bool = True  # MJLab: required by CommandTermCfg

    pose_range: dict[str, tuple[float, float]] = field(default_factory=dict)  # MJLab: {} → field(default_factory=dict)
    velocity_range: dict[str, tuple[float, float]] = field(default_factory=dict)  # MJLab: {} → field(default_factory=dict)
    ...
    # MJLab: VisualizationMarkersCfg → not available; placeholder types
    ...
    target_point_marker_cfg: any = None  # MJLab: VisualizationMarkersCfg | None → any
    target_destination_marker_cfg: any = None  # MJLab: VisualizationMarkersCfg | None → any
    ...
    def build(self, env: ManagerBasedRlEnv) -> MotionCommand:  # MJLab: required by CommandTermCfg abstract method
